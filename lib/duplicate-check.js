...
module.exports = function( redisClient, options ){
    ...
    var defaultOptions = {
        ...
        ignoredProperties: options.ignoreProperties || []
    };
    ...
    function _buildHash( req ){
        ...
        if( keys.length === 0 && defaultOptions.ignoreEmptyBody ){
            return '';
        }

        if( defaultOptions.ignoredProperties.length > 0 ){
            var filteredKeys = keys.filter( function( key ){
                return typeof request[ key ] === 'object';
            } );
            defaultOptions.ignoredProperties.forEach( function( prop ){
                if( request[ prop ] ){
                    delete request[ prop ];
                }
                // Deep check
