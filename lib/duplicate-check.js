...
DuplicateCheck.prototype.middleware = function( options ){
    ...
    if( !options ){
        ...
        options = {
            ...
            infoLogFunc: options.infoLogFunc || false,
            errorLogFunc: options.errorLogFunc || false,
            ovrLogFunc: options.ovrLogFunc || false,
            customDupMsg: options.customDupMsg || 'Duplicate request detected',
            customErrMsg: options.customErrMsg || 'Internal server error has occurred'
        }
    }

    return function( req, res, next ){
        if( req.headers.hasOwnProperty( 'X-Override-DupCheck' ) ){
            if( !_.isEmpty( options.ovrLogFunc ) ){
            ...
            try{
                ...
                debugger;
                if( !_.isEmpty( options.errorLogFunc ) ){
                    options.errorLogFunc( err );
                }
                return res.send( 500, { errors: [ options.customErrMsg ] } );
            }
            _this.raiseFlag( allowOneKey, function( err ){
                ...
                _this.checkHash( redisKey, hash, allowOneKey, function( error, isDuplicate ){
                    if( !!error ){
                        ...
                        if( !_.isEmpty( options.infoLogFunc ) ){
                            options.infoLogFunc( req );
                        }
                        return res.send( 409, { errors: [ options.customDupMsg ] } );
                    }
                    else{
