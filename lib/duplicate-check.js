...
function DuplicateCheck( redisClient, options ){
    ...
    if( !_.isEmpty( options.LoggerFunction ) ){
        this.logger = options.LoggerFunction();
    }
        ...
        _this.raiseFlag( allowOneKey, function( err ){
            ...
            _this.checkHash( redisKey, hash, allowOneKey, function( error, isDuplicate ){
                if( !!error ){
                    ...
                    _this.logger.info( req.href() + " : " + req.params + " : " + 'Duplicate request detected', req, null, { allowOneKey: allowOneKey } );
